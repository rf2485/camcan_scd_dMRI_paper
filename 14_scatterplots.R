...
df_FA <- df 
df_FA$mean_FA_r_lower_cingulum_mask <- remove_outliers(df$mean_FA_r_lower_cingulum_mask)
df_FA <- df_FA %>% drop_na(mean_FA_r_lower_cingulum_mask)
df_MD <- df 
df_MD$mean_MD_r_lower_cingulum_mask <- remove_outliers(df$mean_MD_r_lower_cingulum_mask)
df_MD <- df_MD %>% drop_na(mean_MD_r_lower_cingulum_mask)
df_L1 <- df 
df_L1$mean_L1_r_lower_cingulum_mask <- remove_outliers(df$mean_L1_r_lower_cingulum_mask)
df_L1 <- df_L1 %>% drop_na(mean_L1_r_lower_cingulum_mask)
df_RD <- df 
df_RD$mean_RD_r_lower_cingulum_mask <- remove_outliers(df$mean_RD_r_lower_cingulum_mask)
df_RD <- df_RD %>% drop_na(mean_RD_r_lower_cingulum_mask)
df_ISOVF <- df 
df_ISOVF$mean_ISOVF_r_lower_cingulum_mask <- remove_outliers(df$mean_ISOVF_r_lower_cingulum_mask)
df_ISOVF <- df_ISOVF %>% drop_na(mean_ISOVF_r_lower_cingulum_mask)
df_ICVF <- df 
df_ICVF$mean_ICVF_r_lower_cingulum_mask <- remove_outliers(df$mean_ICVF_r_lower_cingulum_mask)
df_ICVF <- df_ICVF %>% drop_na(mean_ICVF_r_lower_cingulum_mask)
df_OD <- df 
df_OD$mean_OD_r_lower_cingulum_mask <- remove_outliers(df$mean_OD_r_lower_cingulum_mask)
df_OD <- df_OD %>% drop_na(mean_OD_r_lower_cingulum_mask)

### scd-story interaction scatterplots ###

FA_r_lower_cingulum_scd_story_d <- lm(mean_FA_r_lower_cingulum_mask ~ story_d * cohort, df_FA)
summary(FA_r_lower_cingulum_scd_story_d)
fig1FA <- ggplot(df_FA, aes(story_d, mean_FA_r_lower_cingulum_mask, color = cohort)) +
  geom_point() +
...
MD_r_lower_cingulum_scd_story_d <- lm(mean_MD_r_lower_cingulum_mask ~ story_d * cohort, df_MD)
summary(MD_r_lower_cingulum_scd_story_d)
fig1MD <- ggplot(df_MD, aes(story_d, mean_MD_r_lower_cingulum_mask, color = cohort)) +
  geom_point() +
  geom_smooth(method = 'lm', formula = y ~ x) +
  stat_poly_eq(use_label("P"), small.p = T, formula = y ~ x, label.x = "right") +
...
L1_r_lower_cingulum_scd_story_d <- lm(mean_L1_r_lower_cingulum_mask ~ story_d * cohort, df_L1)
summary(L1_r_lower_cingulum_scd_story_d)
fig1AD <- ggplot(df_L1, aes(story_d, mean_L1_r_lower_cingulum_mask, color = cohort)) +
  geom_point() +
  geom_smooth(method = 'lm', formula = y ~ x) +
  stat_poly_eq(use_label("P"), small.p = T, formula = y ~ x, label.x = "right") +
...
RD_r_lower_cingulum_scd_story_d <- lm(mean_RD_r_lower_cingulum_mask ~ story_d * cohort, df_RD)
summary(RD_r_lower_cingulum_scd_story_d)
fig1RD <- ggplot(df_RD, aes(story_d, mean_RD_r_lower_cingulum_mask, color = cohort)) +
  geom_point() +
  geom_smooth(method = 'lm', formula = y ~ x) +
  stat_poly_eq(use_label("P"), small.p = T, formula = y ~ x, label.x = "right") +
...
ISOVF_r_lower_cingulum_scd_story_d <- lm(mean_ISOVF_r_lower_cingulum_mask ~ story_d * cohort, df_ISOVF)
summary(ISOVF_r_lower_cingulum_scd_story_d)
fig1FWVF <- ggplot(df_ISOVF, aes(story_d, mean_ISOVF_r_lower_cingulum_mask, color = cohort)) +
  geom_point() +
  geom_smooth(method = 'lm', formula = y ~ x) +
  stat_poly_eq(use_label("P"), small.p = T, formula = y ~ x, label.x = "right") +
  labs(x = "Story Delayed Recall", y = "Mean ISO",
       color = "Cohort", title = "Right Mean ISO",
       subtitle = paste0(
         "interaction p = ", 
...
ICVF_r_lower_cingulum_scd_story_d <- lm(mean_ICVF_r_lower_cingulum_mask ~ story_d * cohort, df_ICVF)
summary(ICVF_r_lower_cingulum_scd_story_d)
fig1ICVF <- ggplot(df_ICVF, aes(story_d, mean_ICVF_r_lower_cingulum_mask, color = cohort)) +
  geom_point() +
  geom_smooth(method = 'lm', formula = y ~ x) +
  stat_poly_eq(use_label("P"), small.p = T, formula = y ~ x, label.x = "right") +
  labs(x = "Story Delayed Recall", y = "Mean ICVF",
       color = "Cohort", title = "Right Mean ICVF",
       subtitle = paste0(
         "interaction p = ", 
         signif(summary(ICVF_r_lower_cingulum_scd_story_d)$coefficients[4,4], 2))) +
  scale_color_hue(labels = c("Control", "SCD")) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
...
OD_r_lower_cingulum_scd_story_d <- lm(mean_OD_r_lower_cingulum_mask ~ story_d * cohort, df_OD)
summary(OD_r_lower_cingulum_scd_story_d)
fig1OD <- ggplot(df_OD, aes(story_d, mean_OD_r_lower_cingulum_mask, color = cohort)) +
  geom_point() +
  geom_smooth(method = 'lm', formula = y ~ x) +
  stat_poly_eq(use_label("P"), small.p = T, formula = y ~ x, label.x = "right") +
  labs(x = "Story Delayed Recall", y = "Mean OD",
       color = "Cohort", title = "Right Mean OD",
       subtitle = paste0(
         "interaction p = ", 
         signif(summary(OD_r_lower_cingulum_scd_story_d)$coefficients[4,4], 2))) +
  scale_color_hue(labels = c("Control", "SCD")) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
...
MD_cohort_int_age_gender <- lm(mean_MD_lower_cingulum_mask ~ cohort * age, df_MD)
summary(MD_cohort_int_age_gender) #yes
ctl_MD <- df_MD %>% filter(cohort == 'ctl')
scd_MD <- df_MD %>% filter(cohort == 'scd')
ctl_MD_age_gender <- lm(mean_MD_lower_cingulum_mask ~ age, ctl_MD)
scd_MD_age_gender <- lm(mean_MD_lower_cingulum_mask ~ age, scd_MD)
...
MD_cohort_int_age_gender_plot <- interact_plot(MD_cohort_int_age_gender, pred = age, modx = cohort, 
  ...
  labs(x = "Mean Centered Age", y = "Mean MD", title = "Bilateral Mean MD",
       subtitle = paste0("Corrected for Gender \n",
                         "interaction p = ",
                         signif(summary(MD_cohort_int_age_gender)$coefficients[4,4], 2)
       )
  ) +
